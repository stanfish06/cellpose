...
def read_preprocess_and_segment(
    ...
    try:
        ...
        # somehow there might be stale file handle error
        warnings.warn(f"Could not setup logger: {e}")
        pass
...
def distributed_eval(
    ...
    with tempfile.TemporaryDirectory(
        ...
        box_ids = np.concatenate(box_ids_).astype(
            int
...
def block_face_adjacency_graph(faces, nlabels):
    ...
    return scipy.sparse.coo_matrix(
        (v, (i, j)), shape=(nlabels + 1, nlabels + 1)
    ).tocsr()
